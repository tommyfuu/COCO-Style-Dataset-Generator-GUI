...
def read_JSON_file(f):
    ...
    rects = [{'bbox': x['segmentation'][0], 'class': x['category_id'],
              'image_id': x['image_id']} for x in d['annotations']]
    ...
    for rect in rects:
        ...
        r = patches.Rectangle(
            (x0, y0), x1-x0, y1-y0, linewidth=1, edgecolor='g', facecolor='g', alpha=0.4)

        annotations[img_paths[rect['image_id']]].append(
            {'bbox': r, 'cls': d['classes'][rect['class']-1]})
...
class COCO_dataset_generator(object):
    ...
    def __init__(self, fig, ax, img_dir, classes, model_path, json_file):
        ...
        with open(classes, 'r') as f:
            self.classes, img_paths = sorted([x.strip().split(',')[0] for x in f.readlines(
        ...
        self.keyboard_id = self.fig.canvas.mpl_connect(
            'key_press_event', self.onkeyboard)
        self.selected_poly = False
        self.axsave = plt.axes([0.7, 0.05, 0.1, 0.05])
        ...
        if args['no_feedback']:
            ...
            self.model = modellib.MaskRCNN(mode="inference", model_dir='/'.join(
                args['weights_path'].split('/')[:-2]), config=self.config)
            ...
            for i in range(N):
                ...
                pat = patches.Rectangle((rois[i][1], rois[i][0]), rois[i][3]-rois[i][1],
                                        rois[i][2]-rois[i][0], linewidth=1, edgecolor='r', facecolor='r', alpha=0.4)
    ...
    def save(self, event):
        ...
        data = {'images': self.images[:self.index+1],
                'annotations': self.annotations, 'categories': [], 'classes': self.classes}
        ...
        if not event.inaxes:
            ...
            for i, ((xmin, ymin), (xmax, ymax)) in enumerate(self.existing_patches):
                if xmin <= event.xdata <= xmax and ymin <= event.ydata <= ymax:
                    ...
                    self.RS.extents = [coords[0][0], coords[1]
            ...
            if b[1]-b[0] > 0 and b[3]-b[2] > 0:
                poly = [b[0], b[2], b[0], b[3], b[1],
                        b[3], b[1], b[2], b[0], b[2]]
                area = (b[1]-b[0])*(b[3]-b[2])
                bbox = [b[0], b[2], b[1], b[3]]
                dic2 = {'segmentation': [poly], 'area': area, 'iscrowd': 0, 'image_id': self.index, 'bbox': bbox,
                        'category_id': self.classes.index(self.radio.value_selected)+1, 'id': self.ann_id}
                if dic2 not in self.annotations:
                    self.annotations.append(dic2)
                self.ann_id += 1
                rect = patches.Rectangle(
                    (b[0], b[2]), b[1]-b[0], b[3]-b[2], linewidth=1, edgecolor='g', facecolor='g', alpha=0.4)
...
if __name__ == '__main__':
    ...
    ap.add_argument("-c", "--classes_file", required=True,
                    help="Path to classes file")
    ap.add_argument("-j", "--json_file", required=False,
                    help="Path of JSON file to append dataset to", default=None)
    ap.add_argument("--save_csv", required=False, action="store_true",
                    help="Choose option to save dataset as CSV file annotations.csv")
    ...
    ap.add_argument('-x', "--config_path", default=None,
                    help="Path to Mask RCNN JSON config file")
    ...
    # gen = COCO_dataset_generator(
    #     fig, ax, args['image_file'], args['classes_file'], args['weights_path'], args['json_file'])
    gen = COCO_dataset_generator(
        fig, ax, args['image_dir'], args['classes_file'], args['weights_path'], args['json_file'])
