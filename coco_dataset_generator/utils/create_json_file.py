...
if __name__ == '__main__':
    ...
    ap.add_argument("-o", "--file_path", required=True,
                    help="Path of output file")
    ap.add_argument("-c", "--class_file", required=True,
                    help="Path of file with output classes")
    ap.add_argument("-t", "--type", required=True,
                    help="Type of the image files")
    args = vars(ap.parse_args())

    with open(args['class_file'], 'r') as f:
        classes = sorted([unicodedata.normalize('NFKD', x).strip()
                          for x in f.readlines()])

    images, anns = [], []

    img_paths = [x for x in glob.glob(os.path.join(
        args['image_dir'], '*.'+args['type'])) if os.path.exists(x[:-3]+'txt')]
    ...
    for i, f in enumerate(sorted(glob.glob(os.path.join(os.path.abspath(args['image_dir']), '*.txt')))):
        ...
        while(ptr < len(s)):
            ...
            # cat_id = classes.index(s[ptr].encode(
            #     'utf-8').decode('utf-8-sig'))+1
            # tom edits
            cat_id = classes.index(s[ptr].encode(
            ...
            bbox = [x2, y2, x1, y1]
            dic2 = {'segmentation': poly, 'area': area, 'iscrowd': 0,
                    'image_id': i, 'bbox': bbox, 'category_id': cat_id, 'id': ann_index}
            ann_index += 1
            ptr += 4
            anns.append(dic2)
